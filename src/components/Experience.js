...
const Experience = () => {
    ...
    const submitFormHandler = async(experienceData) => {
        ...
        try {
        const data = await axios.post('http://localhost:3200/api/user/updateExperience', {experienceData, _id: user._id});
    ...
    return (
        <>
            ...
            <SimpleGrid columns={[3,3]} spacing='40px'>
                ...
                onChange={(e)=> setexperienceInfo({...experienceInfo, designation: e.target.value})} 
                ...
                onChange={(e)=> setexperienceInfo({...experienceInfo, duration: e.target.value})} 
                value={experienceInfo.duration}/>
                <Input placeholder='From' type='date' name='from' 
                onChange={(e)=> setexperienceInfo({...experienceInfo, from: e.target.value})} 
                value={experienceInfo.from}/>
                <Input placeholder='To' type='date' name='to' 
                onChange={(e)=> setexperienceInfo({...experienceInfo, to: e.target.value})} 
                value={experienceInfo.to}/>
                <Input placeholder='Technology' name='technology' 
                onChange={(e)=> setexperienceInfo({...experienceInfo, technology: e.target.value})} 
